...
function ClientSignup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${BaseURL}/areesiaAPI/signup.php`, formData);
        ...
        <Grid container spacing={0}>
            <Grid item xs={12} md={6}>
                <div className='Client_Left_Item'>
                  <h1>Sign up</h1>
                  <p>Please enter your information here to sign up.</p>
                  <div className='Client_Signup_Form'>
                      <form onSubmit={handleSubmit} className='Client_Signup_First_Step'>
                        <div className='name-fields'>
                          <WhiteBorderTextField onChange={handleInputChange} className='fields' id="firstName" label="First Name" required/>
                          <WhiteBorderTextField onChange={handleInputChange} className='fields' id="lastName" label="Last Name" required/>
                        </div>
                  ...
                  <Link to={'/client-login'} className='Link'>I Have an account</Link>
                </div>
            </Grid>
            <Grid item xs={0} md={6}>
                <div className='Client_Right_Item'>
                  <div className='signup_welcoming'>
                    <p className='message'><span className='coma'>“</span><br/>Your style adventure starts now.</p>
                  </div>
                </div>
            </Grid>
        </Grid>
