  ...
  return (
    <div  className={`popup ${isOpen ? 'open' : ''}`} onClick={onClose}>
        <div className="popup-container" onClick={e => e.stopPropagation()}>
            <div className='Left_Sleeve_Container'>
                <h4>Upload Design</h4>
                <div className='Left_Sleeve_Mockup'>
                  <div className='design_Mockup'>
                    <div className='upload_Container' onClick={() => document.getElementById('leftSleeveDesignInput').click()}>
                      {design && <img src={design} alt="Design" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                      <input type="file" id="leftSleeveDesignInput" accept="image/jpeg" style={{ display: 'none' }} onChange={handleDesignChange} />
                    </div>
                  </div>
                </div>
